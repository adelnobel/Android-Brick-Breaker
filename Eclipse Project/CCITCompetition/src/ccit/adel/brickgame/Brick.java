...
public class Brick {
	...
	public Brick(float x, float y, int id, GameView view) {
		...
		image = BitmapFactory.decodeResource(
				view.getResources(),
				view.getResources().getIdentifier("bricks_" + (id + 1),
						"drawable", view.getContext().getPackageName()));
		image = Bitmap.createScaledBitmap(image, (view.getWidth() - 16)
				/ xBricks, view.getHeight() / 22, false);
	...
	public static List<Brick> initBricks(GameView v) {
		...
		for (int y = 1; y <= Brick.yBricks; y++) {
			...
			for (int x = 1; x <= Brick.xBricks; x++) {
				...
				bricks.add(brick);
				// canvas.drawBitmap(bricks[current].getImage(), offsetX,
				// offsetY, null);
