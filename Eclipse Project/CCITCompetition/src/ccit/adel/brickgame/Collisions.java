...
public class Collisions 
{
	...
	{
		if(ball.getCurrentY() + ball.getHeight() > paddle.getY() + paddle.getHeight())
		{
			return true;
		}
		return false;
	...
	{
		if(ball.getCurrentX() <= 0 || ball.getCurrentX() + ball.getWidth() >= view.getWidth())
		{
			return true;
		}
		return false;
	...
	public boolean isPaddleCollide(Ball ball)
	{
		if(( ball.getCurrentY() + ball.getHeight() >= paddle.getY() )
				&& (ball.getCurrentX() >= paddle.getX() && ball.getCurrentX() < paddle.getX() + paddle.getWidth()))
		{   
			return true;
		}
		return false;
	...
	public boolean isBrickCollide(Ball ball)
	{
	    for(Brick brick : bricks)
	    {
	        ...
	        double xDistance = Math.abs(ball.getCurrentX() + ball.getRadius() - brick.getX() - brick.getWidth() / 2);
	        double yDistance = Math.abs(ball.getCurrentY() + ball.getRadius() - brick.getY() - brick.getHeight() / 2);
	        
	        
	        double cornerDistance_sq = Math.pow( xDistance - brick.getWidth() / 2 , 2 ) + Math.pow(yDistance - brick.getHeight() / 2, 2);
